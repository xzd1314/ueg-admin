...
const statusMap = { '办理中': 'gold', '已办结': 'green', '审核中': 'blue', '已驳回': 'red' }
...
export default function Services() {
  ...
  const submit = (e) => {
    ...
    setApplicant(''); setSubmitted(true); setTimeout(() => setSubmitted(false), 2000)
  }
    ...
    <div>
        ...
        <div className="panel">
          ...
          <form onSubmit={submit}>
            <div className="field"><label>服务事项</label>
              <select value={sv} onChange={(e) => setSv(e.target.value)}>{SERVICES.map((s) => <option key={s.name}>{s.name}</option>)}</select>
            </div>
            ...
            <div className="field"><label>事项概要</label>
              <textarea placeholder="简述申请需求…（前端演示，不写入后端）" />
            </div>
      ...
      <div className="panel">
        ...
        <div className="grid grid-3" style={{ gap: 16 }}>
          {SERVICES.map((s, i) => (
            <div key={s.name} className="stat-card" style={{ padding: 18 }}>
              <div className="label">{String(i + 1).padStart(2, '0')} · {s.type}</div>
              <div style={{ margin: '9px 0 6px', fontWeight: 600 }}>{s.name}</div>
              <div className="small muted">{s.desc}</div>
              <div className="mt-1 small mono muted">{s.dept}</div>
            </div>
          ))}
        </div>
      </div>
