  ...
  return (
    <div>
      <div className="grid grid-3">
        ...
        <div className="stat-card"><div className="label">本次修订意见</div><div className="value" style={{ color: 'var(--blue)' }}>2,398</div><div className="sub">《流浪地球法》生育条款征集</div></div>
      </div>
        ...
        <div className="table-wrap">
          <table className="data">
            ...
            <tbody>
              {LAW.map((l) => (
                <tr key={l.title}>
                  <td style={{ minWidth: 230 }}><b>{l.title}</b><div className="mono small muted">{l.en}</div></td>
                  <td><span className="badge gold">{l.type}</span></td>
                  <td className="mono small">{l.year}</td>
                  <td className="small muted">{l.note}</td>
                  <td><button className="btn ghost sm"><Icon name="bookOpen" size={14} />全文</button></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="section-gap" />
      <div className="alert-box">意见征集通道开放中：对《流浪地球法》生育条款、地下城扩建方案有意见的官员，可通过「公文与通告」页提交。前端演示，不写入真实数据库。</div>
    </div>
  )
}
